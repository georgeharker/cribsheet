...
# the SessionStart hook then silently emitted no instructions). Re-synced on
# every release so the copies cannot drift; the OpenCode npm prepack keeps its
# own copy step as a belt for ad-hoc publishes. (rm first: cp onto a symlink
# would write THROUGH it instead.)
INSTRS=()
