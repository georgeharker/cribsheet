...
def main(argv=None) -> int:
    ...
    def rank_exact(f, order, rel, tset):
        for r, i in enumerate(order, 1):
            ...
            if (rel is not None and fq == rel) or (
                tset and any(_leaf(fq) == _leaf(t) or fq.endswith(t) for t in tset)
            ):
                return r
        return None
    ...
    strat: dict[str, list] = {
        k: [] for k in ["dense", "gatedBM", "blend", "oracle", "covRouter"]
    }
    rer_ranks = []
    ...
    for p, rel, q, tset in items:
        ...
        blend = {
            i: mm(dense).get(i, 0) + args.w * mm(gated).get(i, 0) for i in range(f.n)
        }
        ...
        strat["covRouter"].append(
            rg if top_cov > 0 and rg and rg <= (rd or 999) else rd
        )
        if args.rerank:
            base = [f.by[f.ids[i]]["fqname"] for i in b_order[:20]]
            by = {f.by[i]["fqname"]: f.by[i] for i in f.ids}
            rs = crib.reranker.scores(
                q, [f"{x}: {by[x].get('description', '')}" for x in base]
            )
            ro = [
                base[j]
                for j in sorted(range(len(base)), key=lambda j: rs[j], reverse=True)
            ]
            fused = reciprocal_rank_fusion([base, ro])
            rr = None
            for r, fq in enumerate(fused, 1):
                if (rel is not None and fq == rel) or (
                    tset and any(_leaf(fq) == _leaf(t) for t in tset)
                ):
                    rr = r
                    break
            rer_ranks.append(rr)
    ...
    def h3(rs):
        return sum(1 for r in rs if r and r <= 3) / len(rs)

    def p1(rs):
        return sum(1 for r in rs if r == 1) / len(rs)

    def mrr(rs):
        return sum((1 / r) for r in rs if r) / len(rs)

    print(
        f"n = {len(items)} queries ({n_large} vocab-shifted + {len(orig)} original concept), "
    ...
    if args.rerank:
        print(
            f"{'blend+rr':<12}{p1(rer_ranks):>7.2f}{h3(rer_ranks):>7.2f}{mrr(rer_ranks):>8.3f}"
        )

    # blend justification: how often does blend beat the better single source?
    better = sum(
        1 for a, b in zip(strat["blend"], strat["oracle"]) if a and (not b or a < b)
    )
