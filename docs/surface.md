...
| ---------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **read**   | `lookup`/`apropos`/`read`/… and the `code`+`learning` verbs                      | explicit `project` → the sticky _current_ project → seeded from a path's `.crib`                                                                                                        |
...
_read_ policy (a learning is about a symbol in the code project you're in), and the
two **imports** use _source_ and REQUIRE a `project`/`project_path` — an import is
...
| `crib note elaborate <label> [rel]`              | `note_elaborate`     | Generate per-section _keyword search terms_ (synonyms + phrases a searcher would type) to strengthen BM25 matching. Not prose expansion.                                                                |
| `crib note summarize <label> [rel]`              | `note_summarize`     | Generate per-section _rephrasings_ embedded as dense aliases, so differently-worded queries still match.                                                                                                |
...
_(Concepts and rationale in depth: [decisions-and-plans.md](decisions-and-plans.md).)_
...
Taint is **coarse by design**: it means _a dep moved_, not _this is wrong_. The
...
**Refs resolve by id, relpath, or title** — and a ref that exists in the _other_
...
listed (the sole exception is a doc with no headings, whose body _is_ its one
...
`tainted: true` — the agent reasoning _from_ a stale decision is told at the moment
...
_tainted_ or _proposed_ (an untainted, promoted decision is stable ground; a tainted
...
**Two stalenesses, two claims.** The _decision's own_ taint blocks dependents until
someone `design reaffirm`s the decision. Separately, each item records the hash of
every dep **as it read when the edge was made**, so the _item_ shows ⚠︎ stale in
...
| `crib plan read <ref>`                  | `plan_read`       | **Dossier**: body + status + every dep and dependent annotated, plus the thing a decision has no analog for — `blocked_by`, what the item waits on right now and why. The `plan list` picture for one item, plus its body; the plan-side `design_read`.                                                                                                                                                            |
| `crib plan edit <ref> [body]`           | `plan_edit`       | Rewrite an item's body through the facet; the result carries `newly_tainted` — the items the change put out of date, computed against the pre-edit state. `--source` replaces its citations. The plan-side `design_edit`.                                                                                                                                                                                          |
| `crib plan append <ref> [text]`         | `plan_append`     | Extend an item's body, same edge-aware answer. A body is optional, so an append onto a title-only item **sets** it rather than prepending blanks. `--source` (repeatable) **adds** citations post-hoc, existing ones keeping their capture-time hashes.                                                                                                                                                            |
...
| `crib project index`      | `project_index`     | (Re)index the repo's code AND in-situ docs from `.crib` (cheap re-run via the content-hash gate). The code-only onboard. Streams live progress (`proj: 42/317 files (13%) · 2.1/s · eta 2:05`) — MCP progress notifications (rate/eta lets a client size its call timeout), a stderr ticker on the CLI (TTY-gated, silent when piped). `wait_s=<sec>` bounds the CALL: the final result if the sweep fits, else a partial handle (`running: true`, done/total) while the sweep keeps running daemon-side; re-invoke to JOIN (never a second sweep). |
| `crib project wait`       | `project_wait`      | Bounded-block on a running index job — the JOIN primitive. Final result if the sweep completes within wait_s (default 30s); else the live partial state; call again to keep waiting. `{running: false}` when idle. Never starts a sweep.                                                                                                                                                                                                                                                                                                            |
| `crib project cancel`     | `project_cancel`    | Cancel a running index sweep. Completed files remain (hash-gated); undescribed files retry on the next index. Returns done-at-cancel.                                                                                                                                                                                                                                                                                                                                                                                                               |
| `crib ops`                | `ops`               | The ops view: live sweeps (done/total/failed, rate, eta), per-project in-flight files, and generation slot occupancy (in-flight / waiting / 429s / cooldown seconds). The one call that answers "what is running and why is nothing moving."                                                                                                                                                                                                                                                                                                        |
...
- **Nothing is reindexed.** `chunk_id` is derived from _project-relative_ paths, so
