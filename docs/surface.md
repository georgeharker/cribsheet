...
| `crib project index`      | `project_index`     | (Re)index the repo's code AND in-situ docs from `.crib` (cheap re-run via the content-hash gate). The code-only onboard. Streams live progress (`proj: 42/317 files (13%) · 2.1/s · eta 2:05`) — MCP progress notifications (rate/eta lets a client size its call timeout), a stderr ticker on the CLI (TTY-gated, silent when piped). |
