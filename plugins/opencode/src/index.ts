...
// release: cribsheet consumes sharedserver rather than shipping it, so version lockstep
    ...
    const configHook = async (cfg: {
        ...
        if (register) {
            ...
            if (cfg.mcp[mcpName]) {
                ...
                // Present the inbound-auth bearer when CRIBSHEET_AUTH_TOKEN is set —
                // the backend (server.py) enforces the same token. This hook runs
                // in-process, so unlike a Claude Code headersHelper it reads the token
                // from the env directly. Unset ⇒ no header, open exactly as before.
                const authToken = env.CRIBSHEET_AUTH_TOKEN?.trim()
