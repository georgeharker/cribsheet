"""Optional inbound bearer auth for the cribsheet daemon.

``crib/inbound_auth.py`` is vendored byte-identical from mcp-companion's combiner.
These tests pin the two axes that matter:

- **auth OFF** (``CRIBSHEET_AUTH_TOKEN`` unset) → the daemon behaves exactly as
  before: ``/mcp`` is open, no header required.
- **auth ON** → ``/mcp`` requires the bearer (401 without / wrong, 200 with),
  ``/health`` stays open, and the ``crib`` CLI presents the token on its
  connection.

The middleware is exercised over a trivial Starlette app (no daemon spin-up); the
CLI path is checked by inspecting the transport the ``DaemonClient`` builds.
"""
...
ENV = "CRIBSHEET_AUTH_TOKEN"
TOKEN = "crib-secret-123"
...
def _no_ambient_token(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep an ambient CRIBSHEET_AUTH_TOKEN (dev shell / secrets injection) out of
...
# --- crib CLI presents the bearer ---------------------------------------------


def _daemon_client() -> Any:
    from crib.client import DaemonClient
    from crib.config import DaemonConfig

    cfg = DaemonConfig(enabled=True, name="crib", host="127.0.0.1", port=7732, grace_period="30m")
    return DaemonClient(cfg)


def test_cli_presents_bearer_when_set(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(ENV, TOKEN)
    client = _daemon_client()._make_client()
    assert client.transport.headers.get("Authorization") == f"Bearer {TOKEN}"


def test_cli_sends_no_bearer_when_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(ENV, raising=False)
    client = _daemon_client()._make_client()
    assert not (getattr(client.transport, "headers", None) or {}).get("Authorization")
